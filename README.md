...
This is a personal copy of Ashton Wiersdorf's Emacs Bedrock version 1.3.1. I keep my stuff on GitHub not sourcehut so I did a clone, re-init, and push under the new silly name. I always figured there was an Al Prufrock wandering somewhere around Bedrock, so there it is.

Doom Emacs was too heavy a price to pay for evil-mode. My other configuration attempts have grown crufty and this looks like a good starting point for yet-another-emacs-init-adventure.

I plan to fit my changes (for Fortran, Pascal, Guile mostly) into the same shape/structure as the rest of Bedrock. I do not expect to create anything that would benefit the upstream repository so there was no need to preserve its history. On the off chance I do create something worth sharing, diff is a friend of mine.

Wiersdorf's license and copyright are preserved and included in this repository. Anything I write is always released under the unlicense.
